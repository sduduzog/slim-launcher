...
abstract class BaseFragment : Fragment() {
    ...
    override fun onResume() {
        ...
        val settings = (context
                ?: return).getSharedPreferences(getString(R.string.prefs_settings), AppCompatActivity.MODE_PRIVATE)
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            when (active) {
                0, 3, 5 -> {
                    val flags = (activity
                            ?: return).window.decorView.systemUiVisibility or View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR
                    getFragmentView().systemUiVisibility = flags
                }

            }
            val value = TypedValue()
            (context ?: return).theme.resolveAttribute(R.attr.colorPrimary, value, true)
            (activity ?: return).window.statusBarColor = value.data
        }
